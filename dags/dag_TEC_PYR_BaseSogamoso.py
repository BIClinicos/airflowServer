...
filename = 'TEC_PYR_OportunidadSogamoso.xlsx'
db_tmp_table_dim = 'tmp_dim_patients'
db_table_dim = 'dim_patients'
db_table_fact = "fact_appointments_bw_compensar"
db_tmp_table_fact = "tmp_fact_appointments_bw_compensar"
dag_name = 'dag_' + 'TEC_PYR_BaseSogamoso'
...
def func_get_TEC_PYR_BaseSogamoso ():    
    ...
    print('before_drop_duplicates df_patients',df_patients)
    df_patients = df_patients.drop_duplicates(subset=['document_number','document_type'])
    print('after_drop_duplicates df_patients',df_patients)
    
    print('before_drop_duplicates df_fact_appointments_bw_compensar',df_fact_appointments_bw_compensar)
    ...
    print('after_drop_duplicates df_fact_appointments_bw_compensar',df_fact_appointments_bw_compensar)
    ...
    if ~df.empty and len(df_fact_appointments_bw_compensar.columns) >0:
        load_df_to_sql(df_fact_appointments_bw_compensar, db_tmp_table_fact, sql_connid)
...
with DAG(dag_name,
    ...
    get_TEC_PYR_BaseSogamoso_python_task = PythonOperator(task_id = "get_TEC_PYR_BaseSogamoso",
                                                  python_callable = func_get_TEC_PYR_BaseSogamoso,
                                                  email_on_failure=True, 
                                                  email='BI@clinicos.com.co'
                                                  )
    
   # Se declara la función encargada de ejecutar el "Stored Procedure"
    load_TEC_PYR_BaseSogamoso_dim = MsSqlOperator(task_id='Load_TEC_PYR_BaseSogamoso_dim',
    ...
    # Se declara la función encargada de ejecutar el "Stored Procedure"
    load_TEC_PYR_BaseSogamoso_fact = MsSqlOperator(task_id='Load_TEC_PYR_BaseSogamoso_fact',
                                       mssql_conn_id=sql_connid,
                                       autocommit=True,
...
start_task >> get_TEC_PYR_BaseSogamoso_python_task >> load_TEC_PYR_BaseSogamoso_dim >> load_TEC_PYR_BaseSogamoso_fact >>task_end
# start_task >> get_TEC_PYR_BaseSogamoso_python_task >> load_TEC_PYR_BaseSogamoso_dim >> task_end