...
db_tmp_table = 'tmp_honorario_citas_programadas_staging'
db_table = "Fact_Honorarios_Citas_Programadas"
...
def func_get_honorarios_stating ():
    ...
    domiConsultas_query = f"""
                    ...
                    SELECT TODOX.* FROM (
                        ...
                        LEFT JOIN (SELECT DISTINCT idAppointmenScheduler,idUserProfessional,Tipo_Pago_Gomedisys,Tarifa_Gomedisys,MAX(Fecha_Tarifa_Gomedisys) AS Fecha_Tarifa_Gomedisys  from (
                        SELECT DISTINCT ASCH.idAppointmenScheduler,ASCH.idUserProfessional,BMFD.dateRecord as Fecha_Tarifa_Gomedisys,ASCH.dateBegin,ASCH.dateEnd,
                        ...
                        ) AS Todo GROUP BY Todo.idAppointmenScheduler,Todo.idUserProfessional,Todo.dateBegin,Todo.dateEnd,Todo.Tipo_Pago_Gomedisys,Todo.Tarifa_Gomedisys) AS Professional ON ASCH.idAppointmenScheduler=Professional.idAppointmenScheduler AND ASCH.idUserProfessional=Professional.idUserProfessional
                        )
                    UNION ALL
                    (
                        ...
                        FROM dbo.appointmentSchedulers ASCH  WITH (NOLOCK)
                        INNER JOIN dbo.appointmentSchedulerSlots ASS WITH (NOLOCK) ON ASCH.idAppointmenScheduler=ASS.idAppointmentScheduler
                        LEFT JOIN (SELECT DISTINCT idAppointmenScheduler,idAssistantSpecialist,Tipo_Pago_Gomedisys,Tarifa_Gomedisys,MAX(Fecha_Tarifa_Gomedisys) AS Fecha_Tarifa_Gomedisys from (
                        SELECT DISTINCT ASCH.idAppointmenScheduler,ASCH.idAssistantSpecialist,BMFD.dateRecord as Fecha_Tarifa_Gomedisys,ASCH.dateBegin,ASCH.dateEnd,
                        CASE WHEN BMFD.calculateForm='T' THEN 'Minutos'
                            WHEN BMFD.calculateForm='V' THEN 'Valor' 
                            WHEN BMFD.calculateForm='P' THEN 'Porcentaje' 
                            ELSE '-1'
                        ...
                        INNER JOIN  dbo.billMedicalFeeDetails BMFD WITH (NOLOCK) ON BMF.idMedicalFee=BMFD.idMedicalFee
                        WHERE ASCH.dateBegin BETWEEN BMFD.dateBegin  and BMFD.dateEnd
                        ) AS Todo GROUP BY Todo.idAppointmenScheduler,Todo.idAssistantSpecialist,Todo.dateBegin,Todo.dateEnd,Todo.Tipo_Pago_Gomedisys,Todo.Tarifa_Gomedisys) AS Professional ON ASCH.idAppointmenScheduler=Professional.idAppointmenScheduler AND ASCH.idAssistantSpecialist=Professional.idAssistantSpecialist
...
def execute_Sql():
     query = f"""
     delete from tmp_honorario_citas_programadas_staging where Fecha_Inicio >='{last_week}' AND Fecha_Inicio <'{now}'
     """
...
with DAG(dag_name,
    ...
    load_fact_Honorarios_Citas_programadas = MsSqlOperator(task_id='load_fact_Honorarios_Citas_programadas',
                                          ...
                                          autocommit=True,
                                          sql="EXECUTE sp_load_Fact_Honorarios_Citas_Programadas",
                                         dag=dag
                                         )
