...
def charge_tables(dates2):
    ...
    df_prescripcion = df_servicios_complementarios = df_meds = df_principios_act = df_principios_nutri = df_procedimientos = pd.DataFrame()
    try:
        for date in dates2[:10]:
            url=f'https://wsmipres.sispro.gov.co/WSMIPRESNOPBS/api/Prescripcion/{nit}/{date}/{token}'
            while True:
                try:
                    req = requests.get(url, verify=False, timeout=300)
                    break
            ...
            # Prescripcion
            df_prescripcion = pd.concat([pd.json_normalize(data["prescripcion"]) for data in res], ignore_index=True)

            # Serv compl
            desc_data = pd.json_normalize(res, ["serviciosComplementarios"],[["prescripcion","NoPrescripcion"]],  errors='ignore')
            df_servicios_complementarios = pd.concat([df_servicios_complementarios,desc_data], ignore_index=True)
            
            # Medicamentos
            med_data = pd.json_normalize(res, ["medicamentos"],[["prescripcion","NoPrescripcion"]], errors='ignore')
            if "PrincipiosActivos" in med_data.columns:
                df_meds = pd.concat([df_meds,med_data.drop(["PrincipiosActivos"], axis=1)], ignore_index=True)

            ## Principios activos
            df_principios_act = pd.concat([df_principios_act,
                            pd.json_normalize(med_data.to_dict(orient='records'), 'PrincipiosActivos',["prescripcion.NoPrescripcion"], errors='ignore')],
                            ignore_index=True)

            # Productos nut
            df_principios_nutri = pd.concat([df_principios_nutri,
                            pd.json_normalize(res, ["productosnutricionales"],[["prescripcion","NoPrescripcion"]], errors='ignore')], ignore_index=True)

            # Procedimientos
            if len(res["procedimientos"]) != 0:
                df_procedimientos = pd.concat([df_procedimientos,
                                pd.json_normalize(res, ["procedimientos"],[["prescripcion","NoPrescripcion"]], errors='ignore')], ignore_index=True)

        with pd.ExcelWriter("test.xlsx") as wr:
            df_prescripcion.to_excel(wr,"prescripcion", index=False)
            df_servicios_complementarios.to_excel(wr,"Serv compl", index=False)
            df_meds.to_excel(wr,"Medicamentos", index=False)
            df_principios_act.to_excel(wr,"Principios activos", index=False)
            df_principios_nutri.to_excel(wr,"Productos nut", index=False)
            df_procedimientos.to_excel(wr,"Procedimientos", index=False)
...
dates2 = [date.strftime("%Y-%m-%d") for date in dates]
