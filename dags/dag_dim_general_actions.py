...
def get_data_general_actions():
    ...
    query = f"""
        SELECT GA.idAction,GA.code,GA.name,GA.isActive,GA.dateRecord 
        FROM dbo.generalActions GA WITH (NOLOCK)
        """
    df = sql_2_df(query, sql_conn_id=sql_connid_gomedisys)
    #Convertir a str los campos de tipo fecha 
    cols_dates = ['dateRecord']
...
with DAG(dag_name,
    ...
    load_data_general_actions = MsSqlOperator(task_id="load_data_general_actions",
                                        mssql_conn_id=sql_connid,
                                        autocommit=True,
                                        sql="EXECUTE uspCarga_TblDAcionesGenerales",
                                        email_on_failure=True, 
                                        email='BI@clinicos.com.co',
                                        dag=dag
                                       )
...
start_task >> get_data_general_actions_python_task >> load_data_general_actions >> task_end