...
from 
    TblDusuarios u 
    inner join tblHcitasinasistencia ci on u.idUser = ci.Paciente_id AND ci.contrato_id = 8 and ci.plan_id = 10
        AND convert(date, ci.[Fecha Cita]) between {last_week} AND convert(date, GETDATE())
    ...
    INNER JOIN TblHFormulacionMedicamentos fm on idEncuentro = fm.id_cita AND
    recomendaciones IS NOT NULL and (lower(recomendaciones) like '%o2%' or 
