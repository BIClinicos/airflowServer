select
            u.idUser
            ,u.Documento
            ,ci.PlanTratamiento as Recomendaciones
            ,format(ci.FechaActividad, 'yyyy-MM') date_control
        from 
            TblDusuarios u 
            inner join TblHGomedisysConsultation ci on u.idUser = ci.IdPaciente AND ci.contrato_id = 8 and ci.plan_id = 10 AND
             convert(date, ci.FechaActividad) between {last_week} AND convert(date, GETDATE())
			 AND ci.PlanTratamiento IS NOT NULL and (lower(PlanTratamiento) like '%o2%' or 
            ...
            LEFT JOIN TblHFormulacionMedicamentos fm on ci.idEncounter = fm.id_cita
