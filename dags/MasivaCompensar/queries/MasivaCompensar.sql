...
WITH CTE AS(
    SELECT 
        ...
        (SELECT TOP 1 EVCP.carePlan FROM dbo.EHREventMedicalCarePlan AS EVCP WITH(NOLOCK)
            WHERE EVCP.idEHREvent = EVDom.idEHREvent) AS [EVOLUCIÓN],
                ...
                ISNULL(
					(SELECT TOP 1 Note.note
					FROM dbo.EHREvents AS EVT WITH(NOLOCK)
					...
					ORDER BY EVT.idEHREvent DESC),
					(SELECT TOP 1 Note.note
					FROM dbo.EHREvents AS EVT WITH(NOLOCK)
					INNER JOIN dbo.EHREventNotes AS Note WITH(NOLOCK) ON EVT.idEHREvent = Note.idEHREvent
					WHERE EVT.idEncounter = Enc.idEncounter
					AND EVT.idAction in (98, 350, 394, 1048)
					AND EVT.idSpeciality = 1
					ORDER BY EVT.idEHREvent DESC)
				) AS [SEGUIMIENTO TELEFÓNICO],
    ...
    FROM dbo.encounters AS Enc WITH(NOLOCK)
		...
		--INNER JOIN dbo.EHREvents AS EVFilt WITH(NOLOCK) ON Enc.idEncounter = EVFilt.idEncounter
    ...
    WHERE Enc.idUserCompany = @idUserCompany
		...
		--AND Pat.documentNumber in ('51634932','41462040','19084600','20925194','19176105','20338648','320287','2842741','41510960','11405084','2873692','1010238392','41425151','17162719','4129142','467580','1024613519','17037881','11305535','1032516183','20480160','1093930','79101281')
) SELECT
    [FECHA DEL REGISTRO]
    ,[REGISTRO]
    ,[MES DEL REGISTRO]
